...
/// Hash `(channel_id, ts, text)` to a stable 32-byte digest.
///
/// `ts` is the Slack message timestamp string (e.g. `"1234567890.123456"`).
pub fn message_digest(channel_id: &str, ts: &str, text: &str) -> [u8; 32] {
...
mod tests {
    ...
    fn first_observation_returns_false() {
        ...
        assert!(!c.observe(message_digest("C001", "1234.5678", "hi")));
    }

    #[test]
    fn second_observation_of_same_hash_returns_true() {
        let c = DedupCache::default();
        let h = message_digest("C001", "1234.5678", "hi");
        assert!(!c.observe(h));
        assert!(c.observe(h));
    }
    ...
    fn distinct_messages_do_not_collide() {
        let c = DedupCache::default();
        let a = message_digest("C001", "1234.5678", "hi");
        let b = message_digest("C001", "1234.5678", "bye");
        assert!(!c.observe(a));
        assert!(!c.observe(b));
    }
    ...
    fn capacity_evicts_oldest() {
        ...
        let a = message_digest("C001", "1.1", "a");
        let b = message_digest("C002", "1.2", "b");
        let z = message_digest("C009", "1.9", "z");
        c.observe(a);
        c.observe(b);
